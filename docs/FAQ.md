...
### ¿Qué es el Hilo Fantasma (Zombie Mode) y para qué sirve?
Es un demonio (daemon) interno (`telemetry_daemon_loop`) que evalúa constantemente la salud del sistema. Si el sistema debe simular actividad o enviar latidos (heartbeats) hacia monitores externos (ej. Cloudflare), el Hilo Fantasma asume el control del motor de despacho. En este modo "Zombie", intercepta los lotes de datos y genera métricas de latencias realistas y `job_id`s aleatorios indetectables (ej. 11 dígitos), devolviendo "OK" sin jamás contactar a la API real de Recurso Confiable. Esto permite probar el throughput, mantener dashboards externos en verde, y simular tráfico sin contaminar plataformas de terceros.
